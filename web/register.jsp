    ...
    <div class="login-box-body">
        <form id="dataForm">
            ...
            <div class="row">
                ...
                <!-- /.col -->
                <div class="col-xs-4">
                    <button id="btn_submit" type="button" class="btn btn-primary btn-block btn-flat">注册</button>
                </div>
                <!-- /.col -->
            </div>
        </form>
...
<script>
    ...
    $("#btn_submit").click(function () {
        $.ajax({
            type: "POST",
            url: "/HRM_SSM_war_exploded/register",
            data: $("#dataForm").serialize(),
            success: function (result) {
                if (result == "failure"){
                    $('#modal-message').html("用户名已被注册！");
                    $('#modal-default').modal('show');
                }else{
                    $('#modal-message').html("注册成功！");
                    $('#modal-default').modal('show');
